...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        tdistance = util.Counter()
        distance = [pos.argMax() for pos in livingGhostPositionDistributions]
        d = distance[0]
        for t in distance:
            tdistance[t] = self.distancer.getDistance(pacmanPosition, t)
            if tdistance[d] > tdistance[t]:
                d = t
        actionD = util.Counter()
        act = legal[0]
        for l in legal:
            new_pos = Actions.getSuccessor(pacmanPosition, l)
            actionD[l] = self.distancer.getDistance(new_pos, d)
            if actionD[act] > actionD[l]:
                act = l
        return act